...
import module1.adt.sealed_traits.{Cash, CreditCard, WireTransfer}
...
object adt {
  ...
  object either {
    ...
    val pm: PaymentMethod = Left(CreditCard)
    val pm1: PaymentMethod = Right(Left(WireTransfer))
    val pm2: PaymentMethod = Right(Right(Cash))
